    	...
    	data(){
    		return {
    		   ...
    		   getList(){
		        	...
		        	console.log("您需要的护工星级为："+star);
		        	console.log($this.Page+gender+level);
    		    	 util.Ajax('/api/work/list?_method=POST',{"gender":gender,"level":level,"star":star,"page":$this.Page,"limit":$this.Limit},function(data){
    		    	 	 var data=data.data;
        		  ...
        		  util.Ajax('/api/order/orderCreate?_method=PUT',{
		        		  ...
		        		  "endDate":endDate
	        		  	   ...
	        		  	   if(data.data.status=="0"){//下单成功
	        		  	   	  util.pushRouter(router,url,{"status":data.data.data.status,"orderId":data.data.data.order_id});
	        		  	   }
			          })
        	},
