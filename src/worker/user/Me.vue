<template>
	<div class="Me">
		...
		<div class="me-tab">
			...
			<mt-cell to='/worker/sharing' title="分享">
				<i slot="icon" class="iconfont" style="background:#00B7FF;">&#xe618;</i>
			</mt-cell>
			<mt-cell to='/worker/collect' title="收藏">
				<i slot="icon" class="iconfont" style="background:#F32043;">&#xe62f;</i>
			</mt-cell>
		</div>
	</div>
		...
		mounted: function() {
			...
			util.Ajax('/api/work/isBasicInfoWork/' + this.$store.state.userId, {}, function(data) {
				...
				if($this.infoData.basicInfo_desc == '护工信息已完善') {
					...
					util.Ajax('/api/work/isAuthWork/' + $this.$store.state.userId, {}, function(data) {
						$this.validateData = data.data
						console.log("workStatus:" + $this.validateData.workStatus)
